...
import com.netflix.conductor.tasks.http.providers.MultiValueMapDeserializer;
...
public class HttpTask extends WorkflowSystemTask {
    ...
    @Autowired
    public HttpTask(RestTemplateProvider restTemplateProvider, ObjectMapper objectMapper, JedisProxy jedisProxy) {
        this(TASK_TYPE_HTTP, restTemplateProvider, objectMapper,jedisProxy);
    }

    public HttpTask(
            String name, RestTemplateProvider restTemplateProvider, ObjectMapper objectMapper,JedisProxy jedisProxy) {
        super(name);
    ...
    protected HttpResponse httpCall(Input input) throws Exception {
        ...
        //check is exist expire token
        if (StringUtils.hasLength(input.getTokenExistExpire())){
            try {
                String token = jedisProxy.get(input.getTokenExistExpire());
                if (StringUtils.hasLength(token)){
                    return objectMapper.readValue(token, HttpResponse.class);
                }
            }catch (Exception e){
                LOGGER.error("get redis tenant token fail :{}", e);
            }
        }
        ...
        try {
            ...
            try {
                if (StringUtils.hasLength(input.getTokenExistExpire())){
                    jedisProxy.set(input.getTokenExistExpire(),objectMapper.writeValueAsString(response));
                    jedisProxy.expire(input.getTokenExistExpire(),3000);
                }
            }catch (Exception e){
                LOGGER.error("set redis tenant token fail :{}", e);
            }
    ...
    public static class Input {
        ...
        private String tokenExistExpire;
        ...
        public String getTokenExistExpire() {
            return tokenExistExpire;
        }

        public void setTokenExistExpire(String tokenExistExpire) {
            this.tokenExistExpire = tokenExistExpire;
        }
