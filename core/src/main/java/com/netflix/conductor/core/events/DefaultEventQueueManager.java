...
//        queue.observe().observeOn(Schedulers.io())
//                .flatMap((Message msg) -> Completable.fromAction(() -> {
//                    defaultEventProcessor.handle(queue, msg);
//                }).subscribeOn(Schedulers.io()).toObservable(),20)
//                .observeOn(Schedulers.computation())
//                .subscribe();
        // queue.observe().onBackpressureBuffer()·
        //         .observeOn(Schedulers.io())
        //         .flatMap((Message msg) ->Completable.fromAction(() -> defaultEventProcessor.handle(queue, msg)).subscribeOn(Schedulers.computation()).toObservable().onBackpressureBuffer(),50)
        //         .subscribe();
    }
}
