...
public class WorkflowExecutor {
    ...
    public List<String> getRunningWorkflowIds(String workflowName,String correlationId){
        return executionDAOFacade.getRunningWorkflowIdsByCorrelationId(workflowName,correlationId);
    }
    ...
    public WorkflowModel decide(WorkflowModel workflow) {
        ...
        try {
            ...
            tasksToBeScheduled.forEach(
                    taskModel -> {
                        if(!taskModel.isActiveTimeWindowEnabled()){
                            return;
                        }
                        if(workflow.getVariables().get(taskModel.getNodeType()+"_ATW")!=null){
                            taskModel.addInput("ATW",workflow.getVariables().get(taskModel.getNodeType()+"_ATW"));
                        }
                        if(workflow.getInput().get("entityTimeZone")!=null){
                            taskModel.addInput("entityTimeZone",workflow.getInput().get("entityTimeZone"));
                        }else {
                            if(taskModel.getInputData().get("entityTimeZone")!= null && !taskModel.getInputData().get("entityTimeZone").toString().startsWith("$")){
                                    // means there are filled value
                            }else {
                                taskModel.addInput("entityTimeZone",workflow.getVariables().get(taskModel.getNodeType()+"_fallBackEntityTimeZone"));
                            }
                        }
                    }
            );
    ...
    public void addTaskToQueue(TaskModel task) {
        // put in queue
        String taskQueueName = QueueUtils.getQueueName(task);
        if(task.isActiveTimeWindowEnabled()){
            String activeTimeWindow = task.getInputData().getOrDefault("ATW","").toString(); // e.g. "09:00-18:00"
            String entityTimeZone=  task.getInputData().getOrDefault("entityTimeZone","UTC").toString(); // e.g.  UTC+08:00
            long minDurationUntilActive = (long) 1e15;
            // Parse time window
            String[] timeWindow = activeTimeWindow.split("-");
            String startTimeStr = timeWindow[0].trim();
            String endTimeStr = timeWindow[1].trim();
            ZoneId zoneId;
            try {
                zoneId = ZoneId.of(entityTimeZone);
            } catch (Exception  e) {
                zoneId = ZoneId.of("UTC");
            }
            ZonedDateTime now = ZonedDateTime.now(zoneId);

            ZonedDateTime baseIntervalTime = ZonedDateTime.now(zoneId);
            ZonedDateTime startActiveTime = baseIntervalTime.withHour(Integer.parseInt(startTimeStr.split(":")[0].trim()))
                    .withMinute(Integer.parseInt(startTimeStr.split(":")[1].trim()));
            ZonedDateTime endActiveTime = baseIntervalTime.withHour(Integer.parseInt(endTimeStr.split(":")[0].trim()))
                    .withMinute(Integer.parseInt(endTimeStr.split(":")[1].trim()));
            for (int i = -2; i <= 2; i++) {
                ZonedDateTime adjustedStartActiveTime = startActiveTime.plusDays(i);
                ZonedDateTime adjustedEndActiveTime = endActiveTime.plusDays(i);

                if ((now.isEqual(adjustedStartActiveTime) || now.isAfter(adjustedStartActiveTime)) &&
                        (now.isEqual(adjustedEndActiveTime) || now.isBefore(adjustedEndActiveTime))) {
                    minDurationUntilActive = 0;
                    break;
                } else if (adjustedStartActiveTime.isAfter(now)) {
                    // If now is before the start of the active window, calculate the duration until the window starts
                    long durationUntilStart = ChronoUnit.SECONDS.between(now, adjustedStartActiveTime);
                    minDurationUntilActive = Math.min(minDurationUntilActive, durationUntilStart);
                }
            }

            long waitTimeInSeconds = minDurationUntilActive + Math.max(task.getCallbackAfterSeconds(),0);
            queueDAO.push(
                    taskQueueName,
                    task.getTaskId(),
    ...
    public void updateWorkflow(WorkflowModel workflow){
        executionDAOFacade.updateWorkflowIndex(workflow);
    }
    @VisibleForTesting
