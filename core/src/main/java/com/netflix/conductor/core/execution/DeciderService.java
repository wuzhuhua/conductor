...
public class DeciderService {
    ...
    public DeciderService(
            ...
            @Value("${conductor.app.taskPendingTimeThreshold:60m}")
                    Duration taskPendingTimeThreshold, QueueDAO queueDAO,ConductorProperties properties) {
    ...
    boolean isResponseTimedOut(TaskDef taskDefinition, TaskModel task) {
        ...
        if (pendingTime > thresholdMS) {
            List<String> cleanupIds = properties.getWorkflowCleanupIds()!=null?Arrays.asList(properties.getWorkflowCleanupIds()):Collections.emptyList();
            if(cleanupIds.contains(task.getWorkflowType())) {
                queueDAO.remove(DECIDER_QUEUE, task.getWorkflowInstanceId());
                LOGGER.info("remove decider queue {} for UJ {}",task.getWorkflowInstanceId(),task.getWorkflowType());
            }
            LOGGER.warn(
