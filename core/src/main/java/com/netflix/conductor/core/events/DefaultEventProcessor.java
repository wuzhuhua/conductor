...
import static com.netflix.conductor.common.metadata.events.EventHandler.Action.Type.trigger_task;
...
public class DefaultEventProcessor {
    ...
    public void handle(ObservableQueue queue, Message msg) {
        ...
        try {
            ...
            // Release CPU for a short duration
            try {
        ...
        for (EventHandler eventHandler : eventHandlerList) {
            ...
            if (StringUtils.isNotEmpty(condition) && evaluators.get(evaluatorType) != null) {
                try {
                    ...
                    LOGGER.error("When evaluate the condition: " + condition
                            + " with payload object: " + payloadObject
    ...
    private EventHandler.TaskDetails lookupForCorrespondingTask(EventHandler.TaskDetails triggerTask, Object jsonObject) {
        ...
        if (StringUtils.isNotEmpty(workflowDef) && StringUtils.isNotEmpty(correlationId)) {
            ...
            if(StringUtils.isNotEmpty(TaskSummary.getWorkflowId())  && jsonObject !=null){
    ...
    protected CompletableFuture<List<EventExecution>> executeActionsForEventHandler(
        ...
        for (Action action : eventHandler.getActions()) {
            if(trigger_task.equals(action.getAction())){
                EventHandler.TaskDetails taskDetails=lookupForCorrespondingTask(action.getTrigger_task(),getPayloadObject(msg.getPayload()));
                if(StringUtils.isNotEmpty(taskDetails.getTaskId())){
