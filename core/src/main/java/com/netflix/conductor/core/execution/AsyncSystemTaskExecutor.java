...
public class AsyncSystemTaskExecutor {
     ...
     */
    public void execute(WorkflowSystemTask systemTask, String taskId, String queueNameFrom) {
        TaskModel task = loadTaskQuietly(taskId);
        if (task == null) {
//            LOGGER.error("TaskId: {} could not be found while executing {} from queue {}", taskId, systemTask,queueNameFrom);
            //remove taskId from queue
            queueDAO.remove(queueNameFrom, taskId);
            return;
        }
