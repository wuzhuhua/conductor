...
public class ExecutionService {
    ...
    public TaskSummary getFirstRunningWorkflowIdByCorrelationId(
            String workflowName,
            String correlationId,String referenceTaskName) {
        return executionDAOFacade.getFirstRunningWorkflowIdByCorrelationId(workflowName,correlationId,referenceTaskName);
    }
    ...
    public List<String> getRunningWorkflows(String workflowName, String correlationId) {
    ...
    public long getLatestFinishTimestampByCorrelationId(String correlationId) {
        return executionDAOFacade.getLatestFinishTimestampByCorrelationId(correlationId);
    }

    public List<String> runningWfIdsByCorrelationId(String correlationId) {
        List<String> wfIds = executionDAOFacade.getWfIdsByCorrelationId(correlationId);
        return wfIds.stream().filter(
                wfId -> executionDAOFacade.getWorkflow(wfId, false).getStatus().equals(Workflow.WorkflowStatus.RUNNING)
        ).collect(Collectors.toList());
    }

    public void removePendingWorkflowExecution(String workflowName) {
        executionDAOFacade.removePendingWorkflowAsync(workflowName);
    }

    public void removeWorkflowFromDeciderQueue(String workflowId) {
        executionDAOFacade.removeWorkflowFromDeciderQueue(workflowId);
    }

    public void updateIndexStatus(String workflowId) {
        executionDAOFacade.updateWorkflowIndex(workflowId);
    }

    public Workflow getExecutionStatusFromRedis(String workflowId, boolean includeTasks){
        return executionDAOFacade.getWorkflowFromRedis(workflowId, includeTasks);
    }

    public Object getRedisByKey(String key){
        return executionDAOFacade.getRedisByKey(key);
    }

    public  boolean removeRedis(String taskId,String workflowId){
        return executionDAOFacade.removeRedis(taskId,workflowId);
    }

    public void cleanupRedisByKey(String key,String value,Boolean isHash,Boolean isSet){
        executionDAOFacade.cleanupRedisByKey(key,value,isHash,isSet);
    }
}
