...
public class WorkflowServiceImpl implements WorkflowService {
    ...
    public TaskSummary getFirstRunningWorkflowIdByCorrelationId(
            String name, String correlationId,String referenceTaskName) {
        return executionService.getFirstRunningWorkflowIdByCorrelationId(name, correlationId,referenceTaskName);
    }
    ...
    public List<String> getRunningWorkflows(String workflowName, String correlationId) {
        return workflowExecutor.getRunningWorkflowIds(workflowName, correlationId);
    }
    ...
    public long getLatestFinishTimestampByCorrelationId(String correlationId) {
        long result = executionService.getLatestFinishTimestampByCorrelationId(correlationId);
        if (result == -1) {
            throw new NotFoundException("No finished workflow found for correlationId: %s", correlationId);
        }
        return result;
    }
    ...
    public List<String> runningWfIdsByCorrelationId(String correlationId) {
        return executionService.runningWfIdsByCorrelationId(correlationId);
    }

    @Override
    public void removePendingWorkflowExecution(String workflowName) {
        executionService.removePendingWorkflowExecution(workflowName);
    }

    @Override
    public void removeWorkflowFromDeciderQueue(String workflowId) {
        executionService.removeWorkflowFromDeciderQueue(workflowId);
    }

    @Override
    public void updateIndexStatus(String workflowId) {
        executionService.updateIndexStatus(workflowId);
    }

    @Override
    public Workflow getExecutionStatusFromRedis(String workflowId, boolean includeTasks) {
        Workflow workflow = executionService.getExecutionStatusFromRedis(workflowId, includeTasks);
        if (workflow == null) {
            throw new NotFoundException("Workflow with id: %s not found.", workflowId);
        }
        return workflow;
    ...
    public Object getRedisByKey(String key){
        return executionService.getRedisByKey(key);
    }

    @Override
    public boolean removeRedis(String taskId, String workflowId){
        return executionService.removeRedis(taskId,workflowId);
    }

    @Override
    public void cleanupRedisByKey(String key,String value,Boolean isHash,Boolean isSet){
        executionService.cleanupRedisByKey(key,value,isHash,isSet);
    }
