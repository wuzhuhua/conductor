...
public class TestDefaultEventProcessor {
    ...
    public void setup() {
        ...
        workflowBulkService=mock(WorkflowBulkService.class);
        queue = mock(ObservableQueue.class);
        message =
                new Message(
                        "t0",
                        "{\"Type\":\"Notification\",\"MessageId\":\"7e4e6415-01e9-5caf-abaa-37fd05d446ff\",\"uuid\":\"lijia12138\",\"Message\":\"{\\n    \\\"testKey1\\\": \\\"level1\\\",\\n    \\\"metadata\\\": {\\n      \\\"testKey2\\\": 123456 }\\n  }\",\"Timestamp\":\"2018-08-10T21:22:05.029Z\",\"SignatureVersion\":\"1\"}",
                        "t0");
    ...
    public void testEventHandlerWithCondition() {
        ...
        SimpleActionProcessor actionProcessor =
                new SimpleActionProcessor(
                        ...
                        jsonUtils,
                        startWorkflowOperation,
                        workflowService,workflowBulkService,executionDAOFacade);
    ...
    @Test
    public void testTriggerTaskAction() {
        // setup event handler
        EventHandler eventHandler = new EventHandler();
        eventHandler.setName(UUID.randomUUID().toString());
        eventHandler.setActive(true);
        ...
        Action triggerTaskAction = new Action();
        triggerTaskAction.setAction(Type.trigger_task);
        triggerTaskAction.setTrigger_task(new TaskDetails());
        triggerTaskAction.getTrigger_task().setTaskRefName("task_x");
        triggerTaskAction.getTrigger_task().setWorkflowDef("workflow_x");
        triggerTaskAction.getTrigger_task().setOutput(new HashMap<>());

        eventHandler.getActions().add(triggerTaskAction);
        ...
        taskModel.setReferenceTaskName(triggerTaskAction.getTrigger_task().getTaskRefName());
        WorkflowModel workflow = new WorkflowModel();
        workflow.setTasks(Collections.singletonList(taskModel));
        List<Workflow> workflows = new ArrayList<>();
        Workflow workflow1 = new Workflow();
        workflow1.setCorrelationId(uuid);
        String workflowId = UUID.randomUUID().toString();
        workflow1.setWorkflowId(workflowId);
        Task task = new Task();
        task.setReferenceTaskName("task_x");
        task.setStatus(Task.Status.IN_PROGRESS);
        task.setTaskType(TaskType.TASK_TYPE_HUMAN);
        workflow1.setTasks(Collections.singletonList(task));
        workflows.add(workflow1);

        when(workflowService.getWorkflows(
                        triggerTaskAction.getTrigger_task().getWorkflowDef(), uuid, false, true))
                .thenReturn(workflows);
        when(workflowExecutor.getWorkflow(workflowId, true)).thenReturn(workflow);
        doNothing().when(externalPayloadStorageUtils).verifyAndUpload(any(), any());
