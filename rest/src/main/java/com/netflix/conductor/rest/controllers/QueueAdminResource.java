...
public class QueueAdminResource {
    ...
    public QueueAdminResource(DefaultEventQueueProcessor defaultEventQueueProcessor, EventQueues eventQueues,EventService eventService) {
    ...
    @Operation(summary = "Publish messages in queue")
    @PostMapping("/publish/messages/{eventType}")
    public void publishMessages( @PathVariable("eventType") String eventType, @RequestBody List<Map<String, Object>> payloads) {
        ObservableQueue queue = eventQueues.getQueue(eventType);
        List<Message> messageList = new ArrayList<>();
        ObjectMapper objectMapper=new ObjectMapper();
        List<EventHandler> eventHandlers=eventService.getEventHandlers();
        boolean eventExists = eventHandlers.stream()
                .anyMatch(eventHandler -> eventHandler.getEvent().equals(eventType));
        if (!eventExists) {
            return;
        }
        for (Map<String, Object> item : payloads) {
            try {
                ...
                Message newMessage = new Message(UUID.randomUUID().toString(), payload, null);
