...
public class RedisExecutionDAO extends BaseDynoDAO
    ...
    private static final String CORRELATION_ID_TASK_ID_RUNNING = "CORRELATION_ID_TASK_ID_RUNNING";
    private static final String LATEST_WORKFLOW_FINISH_TIMESTAMP_BY_CORRELATION_ID = "LATEST_WORKFLOW_FINISH_TIMESTAMP_BY_CORRELATION_ID";
    ...
    public void updateTask(TaskModel task) {
        ...
        if(task.getInputData() == null){
            WorkflowModel workflowModel = getWorkflow(task.getWorkflowInstanceId(),true);
            for(TaskModel taskModel: workflowModel.getTasks()){
                if(taskModel.getTaskId().equals(task.getTaskId())){
        ...
        if (taskDefinition.isPresent() && taskDefinition.get().concurrencyLimit() > 0) {
            if (task.getStatus() != null && task.getStatus().equals(TaskModel.Status.IN_PROGRESS)) {
                //                if(TaskType.HUMAN.toString().equals(task.getTaskType()) || TaskType.WAIT.toString().equals(task.getTaskType())){
//                    jedisProxy.zadd(
//                            nsKey(CORRELATION_ID_TASK_ID_RUNNING, task.getWorkflowType(),task.getTaskDefName(), task.getCorrelationId()),task.getStartTime(), task.getTaskId() + ":" + task.getWorkflowInstanceId());
//                }
                jedisProxy.sadd(
                ...
                //                jedisProxy.zrem(nsKey(CORRELATION_ID_TASK_ID_RUNNING, task.getWorkflowType(),task.getTaskDefName(), task.getCorrelationId()), task.getTaskId() + ":" + task.getWorkflowInstanceId());
                jedisProxy.srem(
                        nsKey(TASKS_IN_PROGRESS_STATUS, task.getTaskDefName()), task.getTaskId());
                LOGGER.debug(
    ...
    @Override
    public void removeTaskMappings(TaskModel task) {
        String taskKey = task.getReferenceTaskName() + "" + task.getRetryCount();
    ...
    @Override
    public List<String> getTasksIdForWorkflow(String workflowId) {
        Preconditions.checkNotNull(workflowId, "workflowId cannot be null");
        Set<String> taskIds = jedisProxy.smembers(nsKey(WORKFLOW_TO_TASKS, workflowId));
        recordRedisDaoRequests("getTasksForWorkflow");
    ...
    public boolean removeWorkflowAndTask(String workflowId,List<String> taskIds, int ttlSeconds) {
        WorkflowModel workflow = getWorkflow(workflowId, true);
        if (workflow != null) {
            recordRedisDaoRequests("removeWorkflow");
            // Remove workflow
    ...
    @Override
    public List<String> getWfIdsByCorrelationId(String correlationId, boolean includeTasks) {
        return new ArrayList<>(jedisProxy.smembers(nsKey(CORR_ID_TO_WORKFLOWS, correlationId)));
    }
...
//            }
        ...
        if (workflow.getStatus().isTerminal()) {
            //TODO workflow redis is null
            jedisProxy.srem(
                    nsKey(PENDING_WORKFLOWS, workflow.getWorkflowName()), workflow.getWorkflowId());
//            Long finishedAt = workflow.getEndTime();
//            if (workflow.getCorrelationId() != null) {
//                jedisProxy.set(nsKey(LATEST_WORKFLOW_FINISH_TIMESTAMP_BY_CORRELATION_ID, workflow.getCorrelationId()), String.valueOf(finishedAt));
//            }

        } else {
//            INSIGHT_LAYER_LOGGER.info(InsightLayerLogger.createWorkflowLogEntry(workflow));
            jedisProxy.sadd(
                    nsKey(PENDING_WORKFLOWS, workflow.getWorkflowName()), workflow.getWorkflowId());
        }
    ...
    public long getLatestFinishTimestampByCorrelationId(String correlationId) {
        String value = jedisProxy.get(nsKey(LATEST_WORKFLOW_FINISH_TIMESTAMP_BY_CORRELATION_ID, correlationId));
        return Long.parseLong(value == null ? "-1" : value);
    }

    @Override
    public void removeWorkflowRef(WorkflowModel workflow){
        ...
        jedisProxy.srem(nsKey(PENDING_WORKFLOWS, workflow.getWorkflowName()), workflowId);
        jedisProxy.del(nsKey(SCHEDULED_TASKS, workflowId));
        jedisProxy.del(nsKey(WORKFLOW_TO_TASKS, workflowId));
    }

    @Override
    public void cleanupRedisByKey(String key,String value,Boolean isHash,Boolean isSet){
        if(isHash){
            jedisProxy.hdel(key,value);
        }else if(isSet){
            jedisProxy.srem(key,value);
        }else{
            jedisProxy.del(key);
        }
    }
    ...
    @Override
    public void addTaskCorrIdMappings(TaskModel task){
        jedisProxy.zadd(
                nsKey(CORRELATION_ID_TASK_ID_RUNNING, task.getWorkflowType(),task.getReferenceTaskName(), task.getCorrelationId()),task.getStartTime(), task.getTaskId() + ":" + task.getWorkflowInstanceId());
    }

    @Override
    public void removeTaskCorrIdMappings(TaskModel task){
        jedisProxy.zrem(nsKey(CORRELATION_ID_TASK_ID_RUNNING, task.getWorkflowType(),task.getReferenceTaskName(), task.getCorrelationId()), task.getTaskId() + ":" + task.getWorkflowInstanceId());
    }
    @Override
    public String[] getTaskCorrIdMappings(String workflowType,String taskRefName,String corrId){
        Set<String> results = jedisProxy.zrange(nsKey(CORRELATION_ID_TASK_ID_RUNNING,workflowType, taskRefName, corrId),0,0);
        if (results != null && !results.isEmpty()) {
            String first = results.iterator().next();
            return first.split(":");
        }
        return null;
    }
