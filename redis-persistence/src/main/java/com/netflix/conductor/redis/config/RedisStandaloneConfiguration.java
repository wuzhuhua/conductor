...
public class RedisStandaloneConfiguration extends JedisCommandsConfigurer {
    ...
    protected JedisCommands createJedisCommands(
        ...
        return new JedisStandalone(getJedisPool(config, host,properties.isSSL()));
    }

    private JedisPool getJedisPool(JedisPoolConfig config, Host host, boolean isSSL) {
        if (host.getPassword() != null) {
            ...
            return new JedisPool(config, host.getHostName(), host.getPort(),isSSL);
        }
    }
}
